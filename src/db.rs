...
pub async fn get_latest_recorded_block(pool: &SqlitePool) -> Result<u64, sqlx::Error> {
    Ok(
        sqlx::query!("SELECT block_number FROM block_status ORDER BY block_number DESC LIMIT 1")
            .fetch_optional(pool)
            .await?
...
pub async fn append_block_task(pool: &SqlitePool, block_number: u64) -> Result<(), sqlx::Error> {
    ...
    sqlx::query!(
        "INSERT INTO block_status (block_number) VALUES (?) ON CONFLICT(block_number) DO NOTHING",
        block_number,
    )
    .execute(pool)
    .await?;
...
pub async fn append_tx_task(
    ...
    sqlx::query!(
        "INSERT INTO tx_status (block_number, tx_index, tx_hash) VALUES (?, ?, ?) ON CONFLICT(tx_hash) DO NOTHING",
        block_number,
        tx_index,
        hash,
    )
...
pub async fn acquire_block_task(pool: &SqlitePool) -> Result<Option<u64>, sqlx::Error> {
    let block_number =
        sqlx::query!("SELECT block_number FROM block_status WHERE tx_fetched = 0 LIMIT 1",)
            .fetch_optional(pool)
            .await?
    ...
    sqlx::query!(
        "UPDATE block_status SET tx_fetched = 2 WHERE block_number = ?",
        block_number,
    )
    .execute(pool)
    .await?;
...
pub async fn mark_block_task_fetched(
    ...
    let block_number = block_number as i64;
    sqlx::query!(
        "UPDATE block_status SET tx_fetched = 1 WHERE block_number = ? AND tx_fetched = 2",
        block_number,
    )
    .execute(pool)
    .await?;
...
pub async fn acquire_tx_task(pool: &SqlitePool) -> Result<Option<H256>, sqlx::Error> {
    let tx_hash = sqlx::query!("SELECT tx_hash FROM tx_status WHERE tx_analyzed = 0 LIMIT 1",)
        .fetch_optional(pool)
        .await?
        .map(|row| row.tx_hash);
    if tx_hash.is_none() {
    ...
    sqlx::query!(
        "UPDATE tx_status SET tx_analyzed = 2 WHERE tx_hash = ?",
        tx_hash,
    )
    .execute(pool)
    .await?;
...
pub async fn mark_tx_task_analyzed(pool: &SqlitePool, tx_hash: H256) -> Result<(), sqlx::Error> {
    ...
    let result = sqlx::query!(
        "UPDATE tx_status SET tx_analyzed = 1 WHERE tx_hash = ? AND tx_analyzed = 2",
        hash,
    )
    .execute(pool)
    .await?;
...
pub async fn clear_pending_tasks(pool: &SqlitePool) -> Result<(), sqlx::Error> {
    sqlx::query!("UPDATE block_status SET tx_fetched = 0 WHERE tx_fetched = 2",)
        .execute(pool)
        .await?;
    sqlx::query!("UPDATE tx_status SET tx_analyzed = 0 WHERE tx_analyzed = 2",)
        .execute(pool)
        .await?;
    Ok(())
}
